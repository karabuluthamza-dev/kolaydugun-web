-- Vendor Gamification: Rankings & Top 5
-- 1. Function to get vendor rank and competition info
...
-- 2. Function to get Top 5 vendors for a category/city
CREATE OR REPLACE FUNCTION public.get_top_vendors(p_category TEXT, p_city TEXT DEFAULT NULL, p_limit INTEGER DEFAULT 5)
...
BEGIN
    ...
    WHERE category = p_category
      ...
      AND deleted_at IS NULL
      AND ai_performance_score >= 80 -- Only high quality ones for the spotlight
    ORDER BY ai_performance_score DESC, created_at ASC
    LIMIT p_limit;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
