    ...
    return (
        <div className="flex flex-col h-screen bg-[#050505] text-white selection:bg-prime selection:text-white font-sans overflow-hidden">
            ...
            <header className="px-8 py-6 bg-black/40 backdrop-blur-2xl border-b border-white/5 flex items-center justify-between sticky top-0 z-50">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <div className="px-2 py-0.5 bg-green-500/10 border border-green-500/20 rounded-md flex items-center gap-1.5">
                                ...
                                <span className="text-[9px] font-black text-green-500 uppercase tracking-widest">{t('dashboard.eventCard.active')}</span>
                            </div>
                        </div>
                        <p className="text-[11px] font-bold text-white/30 uppercase tracking-[0.2em] ml-0.5 hidden md:block">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <motion.button
                        ...
                        title={soundEnabled ? t('liveFeed.soundOn') : t('liveFeed.soundOff')}
                    >
                    ...
                    <motion.button
                        ...
                        className="px-4 md:px-6 py-4 bg-prime border border-prime/20 rounded-2xl flex items-center gap-3 transition-all hover:bg-prime/90 group shadow-[0_10px_30px_rgba(244,63,94,0.3)]"
                    >
                        <QrCode className="w-5 h-5 text-white group-hover:rotate-12 transition-transform" />
                        <span className="text-[11px] font-black text-white uppercase tracking-[0.2em] hidden md:block">{t('liveFeed.qrLink')}</span>
                    </motion.button>
                </div>
            </header>

            {/* Mobile Tab Switcher */}
            <div className="flex md:hidden bg-black/40 border-b border-white/5 p-2 sticky top-[89px] z-40 backdrop-blur-md">
                <button
                    onClick={() => setActiveTab('pending')}
                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === 'pending' ? 'bg-white/10 text-white' : 'text-white/30'}`}
                >
                    {t('liveFeed.tabs.pending')} ({pendingRequests.length})
                </button>
                <button
                    onClick={() => setActiveTab('played')}
                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === 'played' ? 'bg-white/10 text-white' : 'text-white/30'}`}
                >
                    {t('liveFeed.tabs.played')} ({playedRequests.length})
                </button>
            </div>
            ...
            <main className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-10 space-y-8 bg-[radial-gradient(circle_at_top_right,_#111_0%,_#050505_100%)]">
                                    ...
                                    <div className="flex flex-col lg:flex-row">
                                        ...
                                        <div className="lg:w-[450px] bg-white/[0.02] border-l border-white/5 p-10 md:p-12 flex flex-col justify-between gap-8">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3 text-xs font-black tracking-widest text-white/20 uppercase">{t('liveFeed.searchTools')}</div>
                ...
                {/* History Section - Always visible on desktop, tabbed on mobile */}
                {(activeTab === 'played' || window.innerWidth > 768) && playedRequests.length > 0 && (
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-3xl">
                        ...
                        >
                            ...
                            <div className="flex items-center gap-6 mb-12 relative z-10">
                                ...
                                <div>
                                    <h2 className="text-3xl font-black tracking-tight uppercase tracking-[0.05em]">{t('liveFeed.battle.title')}</h2>
                                    <p className="text-[10px] text-white/40 font-black uppercase tracking-[0.3em] mt-1">{t('liveFeed.battle.subtitle')}</p>
                                </div>
                                <button
                                    ...
                                    <div className="p-8 bg-white/[0.03] rounded-[2.5rem] border border-white/5 backdrop-blur-md">
                                        <div className="flex items-center justify-between mb-8">
                                            ...
                                            <div className="flex items-center gap-2 px-4 py-1.5 bg-green-500/10 border border-green-500/20 rounded-full">
                                                ...
                                                <span className="text-[10px] font-black text-green-500 uppercase tracking-widest">{t('dashboard.eventCard.active')}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-8 pt-8 border-t border-white/5 flex justify-center text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">
                                            {t('liveFeed.battle.totalVotes')}: {battleVotes.A + battleVotes.B}
                                        </div>
                                    </div>
